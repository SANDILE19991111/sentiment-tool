...
if st.session_state.results is not None and st.session_state.original_col is not None:
    ...
    with c1:
        st.markdown(f'<div class="metric-card card-total"><p class="metric-num">{total}</p><p class="metric-label">Total Analysed</p></div>', unsafe_allow_html=True)
    with c2:
        st.markdown(f'<div class="metric-card card-pos"><p class="metric-num">{pos}</p><p class="metric-label">✅ Positive</p></div>', unsafe_allow_html=True)
    with c3:
        st.markdown(f'<div class="metric-card card-neg"><p class="metric-num">{neg}</p><p class="metric-label">❌ Negative</p></div>', unsafe_allow_html=True)
    with c4:
        st.markdown(f'<div class="metric-card card-neu"><p class="metric-num">{neu}</p><p class="metric-label">😐 Neutral</p></div>', unsafe_allow_html=True)
    with c5:
        st.markdown(f'<div class="metric-card card-pol"><p class="metric-num">{avg_pol:+.2f}</p><p class="metric-label">Avg Polarity</p></div>', unsafe_allow_html=True)
    ...
    with col_l:
        ...
        if chart_type == "Bar Chart":
            fig = px.bar(
                ...
                text=counts.values, labels={'x': '', 'y': 'Count'}
            )
    ...
    recs = []
    if neg / total > 0.3:
        recs.append(("bad", "⚠️ High negative rate — investigate common complaints"))
    if pos / total > 0.6:
        recs.append(("good", "🌟 Strong positive sentiment — use this in your marketing"))
    if avg_pol < -0.1:
        recs.append(("bad", "🔴 Overall tone is negative — prioritise improvements"))
    elif avg_pol > 0.1:
        recs.append(("good", "🟢 Overall tone is positive — keep maintaining quality"))
    if not recs:
        recs.append(("warn", "📊 Mixed sentiment — look deeper for specific themes"))
    ...
    st.dataframe(
        display.style.map(colour_sentiment, subset=['Sentiment']),
...
| Avg Confidence | {res['confidence'].mean():.1%} |

## Recommendations
{''.join([f'- {t}{chr(10)}' for _, t in recs])}
...
st.markdown("<br>", unsafe_allow_html=True)
st.markdown(
    "<div style='text-align:center;padding:1.5rem;background:linear-gradient(135deg,#667eea20,#764ba220);border-radius:16px;color:#555;font-size:0.85rem;'>"
    "🧠 <b>SentimentIQ</b> · Built with Python, TextBlob & Streamlit · Bongimusa Khoza · 2026"
    "</div>",
    unsafe_allow_html=True
)
